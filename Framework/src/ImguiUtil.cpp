...
void ImGuiUtil::ShowPanel(uint32_t W, uint32_t H, RENDER_TYPE& type, App* app){
    ...
    if (ImGui::TreeNode("Operation Config")) {
        if (ImGui::BeginTable("Operation", 2, ImGuiTableFlags_None)) {
            ImGui::TableSetupColumn("Label", ImGuiTableColumnFlags_WidthFixed, 80.0f); // 1列目: ラベル
