...
IDxcBlob* CompileShaderLibrary(LPCWSTR fileName/*, LPCWSTR entryPoint*/)
{
  ...
  std::cout << /*sShader*/sShader.length() << std::endl;
  
  // Create blob from the string
  IDxcBlobEncoding* pTextBlob;
  ThrowIfFailed(pLibrary->CreateBlobWithEncodingFromPinned(
      (LPBYTE)sShader.c_str(), (uint32_t)sShader.size(), 65001/*0*/, &pTextBlob));

  // Compile
  IDxcOperationResult* pResult;
  /*ThrowIfFailed(pCompiler->Compile(pTextBlob, fileName, L"", L"lib_6_3", nullptr, 0, nullptr, 0,
                                   dxcIncludeHandler, &pResult));*/
                                   // 1. 引数リストを作成（DXRを有効にし、ターゲットを明示する）
  LPCWSTR args[] = {
      L"-T", L"lib_6_3",      // ターゲットプロファイル
      L"-Qunused-arguments",  // 未使用引数の警告を抑制
  };
  ...
  ThrowIfFailed(pCompiler->Compile(
      pTextBlob,              // ソース
      fileName,               // ファイル名
      L"",                    // Libraryなのでエントリーポイントは空
      L"lib_6_3",             // ターゲット
      args,                   // ← ここに args を渡す
      _countof(args),         // ← ここに引数の数を渡す
      nullptr,                // Defines
      0,                      // Define count
  ...
  if (pResult == nullptr)std::cout << "nulptr : pResult" << std::endl;
  else std::cout << "not nullptr : pResult" << std::endl;
  // Verify the result
  HRESULT resultCode;
